...
public class BoardTestSuite {
    ...
    void testAddTaskListAverageWorkingOnTask() {
        ...
        //When
        List<TaskList> averageDaysTasksFromStartToNow = new ArrayList<>();
        averageDaysTasksFromStartToNow.add(new TaskList("In progress"));

        List<Integer> tasks = project.getTaskLists().stream()
                .filter(averageDaysTasksFromStartToNow::contains)
                .flatMap(r -> r.getTasks().stream())
                .map(t -> t.getCreated().until(LocalDate.now()).getDays())
                .collect(Collectors.toList());
        double average = tasks.stream().mapToInt(task -> task).average().getAsDouble();
        //Then
